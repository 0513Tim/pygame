...
canvas_height = 600
#載入圖片
background_img = pygame.image.load("tetris_background.jpg")
background_img = pygame.transform.scale(background_img, (canvas_width, canvas_height))  # 確保大小符合視窗
# 顏色.
color_block         = (0,0,0)
...
brick_dict = {
    ...
    "80": ( 0, 5,10,15), "81": ( 3, 6, 9,12),    #I.
    "90": ( 9,10,11,12), "91": ( 0, 5, 9,13),  "92": (11,12,13,14), "93": ( 1, 5, 9, 14), 
}
#載入背景音樂
pygame.mixer.init()
...
def brickNew():
    ...
    # 下個出現方塊.
    # 方塊編號(1~8).
    brick_next_id = random.randint( 1, 9)
...
while running:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # 變換方塊-上.
                ...
                if (container_x == 8 ):
                    break
                # 判斷方塊N1、N2、I.
                if (brick_id == 1 or brick_id == 2 or brick_id == 7 or brick_id == 8):
                    # 長條方塊旋轉例外處理.
                    if (brick_id == 7 or brick_id == 8):
                        if (container_x < 1 or container_x == 7):
                            break
                    # 旋轉方塊.
                    ...
                    if (not ifCopyToBricksArray()):
                        ...
                        if (brick_state < 0):
                            brick_state = 1
                # 判斷磚跨L1、L2、T.                                
                elif (brick_id == 3 or brick_id == 4 or brick_id == 5 or brick_id == 9):
                    # 長條方塊旋轉例外處理.
                    if (brick_id == 9):
                        if (container_x < 1 or container_x == 6):
                            break
                    # 旋轉方塊.
                    brick_state = brick_state + 1
                    if (brick_state > 3):
    ...
    if (game_mode == 0):
        # 處理方塊下降.
        # if(lines_number/5>0):
        #     brick_down_speed = BRICK_DOWN_SPEED_MAX - lines_number/5*0.01
    ...
    #---------------------------------------------------------------------    
    canvas.blit(background_img, (0, 0))  # 繪製背景
    # 更新下一個方塊圖形.
        ...
        for y in range(4):
            ...
            for x in range(4):            
                if (bricks[x][y] != 0):
                    ...
                    str_x = str_x + "  "
            pos_x = 15 + (container_x * 20)
            pos_y = 20 + (posY * 28)
            showFont( str_x, pos_x, pos_y, color_blue)

    # 顯示訊息.
    showFont( u"下次出現方塊", 588, 16, color_blue)

    showFont( u"最大連線數", 588, 190, color_blue)
    showFont( str(int(lines_number_max)), 588, 220, color_blue)

    showFont( u"本局連線數", 588, 260, color_blue)
    showFont( str(int(lines_number)), 588, 290, color_blue)
